...
*/
void show_help() {
    cout << "Usage: 1. zhanghongfei -num <number> [options]" << endl;
    cout << "       2. zhanghongfei -indir <path> -impr -ni <number> [options]" << endl << endl;
...
void load_filter_blobs(MainWindow* w) {
    ...
    if (!noise_only) {
        ...
        for (freq_ind = 0; freq_ind < distnct_f; freq_ind++) {
            for (orient_ind = 0; orient_ind < distnct_o; orient_ind++) {
                ...
                {
                    if (is_GUI)
                        QMessageBox::information(w, "zhanghongfei", "Not able to load Filter Bank.", QMessageBox::Ok, QMessageBox::Ok);
                    else
    ...
    {
        if (is_GUI)
            QMessageBox::information(w, "zhanghongfei", "Not able to load Noise Blobs.", QMessageBox::Ok, QMessageBox::Ok);
        else
...
{   
    ...
    //DensityMaps d;
    //OrientMap b;
    //DensityMaps::sel_n_merg_densitymap
    //b.orientmap();
    //d.sel_n_merg_densitymap();
    ...
    if (argc == 1)
    {
        ...
        // zhanghongfei in GUI mode
        is_GUI = true;
        QApplication a(argc, argv);
        //a.setStyle("motif");
        a.setStyle("QWindowsStyle");
