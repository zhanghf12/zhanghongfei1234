...
void Generator::generation(void) {
    int i, j, g_k;
    //cvSaveImage("./FingerPrints/b.jpg", f_print1);
    {
        int rand_num;
    ...
    //cvSaveImage("D:/b.jpg", f_print1);
    noise_obj->rotmax = ((double)rotation * 22) / (7.0 * 180.0);
    ...
    cv :: Mat s(400, 275, CV_32FC1);
    for (i = 0; i < H + margin; i++)
        for (j = 0; j < W + margin; j++)
            //cout << f_print1_2Dmat[i][j] << " ";
            s.at<float>(i, j) = temp_fp_2Dmat[i][j];
    cv::Mat struct1, struct2;
    struct1 = cv::getStructuringElement(0, cv::Size(2, 2));
    struct2 = cv::getStructuringElement(0, cv::Size(2, 2));
    //erode(s, s, struct1);
    //dilate(s, s, struct2);
    if (Wid == 1) {
        dilate(s, s, struct2);
    ...
    for (i = 0; i < H + margin; i++)
        for (j = 0; j < W + margin; j++)
            //cout << f_print1_2Dmat[i][j] << " ";
            temp_fp_2Dmat[i][j] = s.at<float>(i, j);
    mat2DtoIpl(temp_fp_2Dmat, f_print1);

    imwrite(filename.str(), s);
    ...
    cvCopy(f_print1, noise_obj->f_print1, 0);
    ...
    int k;
    cvCopy(noise_obj->f_print1, noise_obj->f_print2, 0);
    ...
    for (k = 0; k < n_impr; k++)
    {
        ...
        cvCopy(noise_obj->f_print2, noise_obj->f_print1, 0);
        Ipltomat2D(noise_obj->f_print1, noise_obj->f_print1_2Dmat);
        for (i = 0; i < noise::H + noise::margin; i++) {
            for (j = 0; j < noise::W + noise::margin; j++) {
                if (noise_obj->f_print1_2Dmat[i][j] > 127)
...
void Generator::set_low_den_para() {
    ...
    cx2 = floor(cx1 + W / 3.0 + 1.0 * (2.0 * W / 3.0 - cx1) - 1.0 * k1);
...
//}
//int main(){
//    int singularity_type = 3;
//    initialize();
//    /*Generator init_para();
//    Generator set_param();
//    Generator seed_pos();
//    OrientMap OrientMap();
//    cvShowImage("image", OrientMap().orient);
//    cvWaitKey(0);*/
