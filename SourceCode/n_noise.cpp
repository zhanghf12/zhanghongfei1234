...
void Noise::random_noise(IplImage* f_print) {
    ...
    cvEllipse(mask, cvPoint(sx, sy - c1 / 2), cvSize(a1, b2), 180, 0, 90,
        cvScalar(255, 255, 255), 1, 8, 0);
    cvEllipse(mask, cvPoint(sx, sy - c1 / 2), cvSize(a2, b2), 180, 90, 180,
        cvScalar(255, 255, 255), 1, 8, 0);
    cvEllipse(mask, cvPoint(sx, sy + c1 / 2), cvSize(a1, b1), 180, 270, 360,
        cvScalar(255, 255, 255), 1, 8, 0);
    cvEllipse(mask, cvPoint(sx, sy + c1 / 2), cvSize(a2, b1), 180, 180, 270,
        cvScalar(255, 255, 255), 1, 8, 0);
    ...
    cvFloodFill(mask, seed_point, color, cvScalarAll(5.0), cvScalarAll(5.0), NULL, 4, NULL);
    ...
    IplImage* f_noise = cvCreateImage(cvSize(W + margin, H + margin), IPL_DEPTH_32F, 1);
    ...
    for (i = 0; i < Narea; i++) {
        max_noise_blob[i] = 4 * 0.0003 * ((i + 1) * (distance_max / Narea)) * ((i + 1) * (distance_max / Narea)) * sqrt((i + 1) * (distance_max / Narea));
    }
    ...
    for (i = 0; i < n_noise; i++) {
        ...
        double distance = sqrt((W / 2.0 - x) * (W / 2 - x) + (H / 2 - y) * (H / 2 - y));
...
void Noise::distortion(const char* filename)
{
    ...
    for (j = 1; j <= new_dim; j++)
    {
        for (k = 1; k <= new_dim; k++)
        {
            ...
            if (!(distortionX <= 1 || distortionY <= 1 || distortionX >= 400 || distortionY >= 400))
            {
    ...
    //cv::Mat m(400, 275, CV_32FC1);
    cv::Mat n(400, 275, CV_8UC1);
    for (i = 0; i < height; i++)
        for (j = 0; j < width; j++)
            //cout << f_print1_2Dmat[i][j] << " ";
            //m.at<float>(i, j) = f_print1_2Dmat[i][j];
            n.at<uchar>(i, j) = f_print1_2Dmat[i][j];
    //cvCopy(&m, &n);
    imwrite(filename, n);
    //cvReleaseImage(&img);
